...
async function updatePricingElements() {
    ...
    corePlanTypes.forEach(planType => {
        ...
        if (monthlyPrice) {
            const formattedMonthlyPrice = (monthlyPrice.amount / Math.pow(10, monthlyPrice.scale)).toFixed(0);
            document.querySelectorAll(`[data-pricing-plan="${planType}"][data-pricing-id="monthly-total"]`)
                .forEach(element => {
                    element.innerHTML = formattedMonthlyPrice;
                });

            // Monthly-annual-total
            const monthlyAnnualTotal = ((monthlyPrice.amount * 12) / Math.pow(10, monthlyPrice.scale)).toFixed(0);
            document.querySelectorAll(`[data-pricing-plan="${planType}"][data-pricing-id="monthly-annual-total"]`)
                .forEach(element => {
                    element.innerHTML = monthlyAnnualTotal;
                });
        }

        // Yearly pricing
        const yearlyPrice = plan?.pricing?.yearly?.discountedPrice;
        if (yearlyPrice) {
            const yearlyPricePerMonth = (yearlyPrice.amount / Math.pow(10, yearlyPrice.scale) / 10).toFixed(0);
            document.querySelectorAll(`[data-pricing-plan="${planType}"][data-pricing-id="yearly-total"]`)
                .forEach(element => {
                    element.innerHTML = yearlyPricePerMonth;
                });
        }
    });
    ...
    growthPlanTypes.forEach(planType => {
        ...
        // Monthly pricing (using discountedPrice)
        const monthlyPrice = plan?.pricing?.monthly?.discountedPrice;
        if (monthlyPrice) {
            const formattedMonthlyPrice = (monthlyPrice.amount / Math.pow(10, monthlyPrice.scale)).toFixed(0);
            document.querySelectorAll(`[data-pricing-plan="${planType}"][data-pricing-id="monthly-total"]`)
                .forEach(element => {
        ...
        // Yearly pricing (using discountedPrice)
        const yearlyPrice = plan?.pricing?.yearly?.discountedPrice;
        if (yearlyPrice) {
            const yearlyPricePerMonth = (yearlyPrice.amount / Math.pow(10, yearlyPrice.scale) / 10).toFixed(0);
            document.querySelectorAll(`[data-pricing-plan="${planType}"][data-pricing-id="yearly-total"]`)
